    ...
    function Searchtools(elem, options) {
      ...
      this.orderCols.forEach(function (item) {
        item.addEventListener('click', function (event) {
          var element = event.target.tagName.toLowerCase() === 'span' ? event.target.parentNode : event.target;
      ...
      document.body.addEventListener('click', function (event) {
        if (document.body.classList.contains('filters-shown')) {
          ...
          if (event.composedPath && typeof event.composedPath === 'function') {
            ...
            if (event.composedPath().indexOf(_this.filterContainer) !== -1) {
              return;
            }
          } else {
            var node = event.target;
            while (node !== document.body) {
              if (node === _this.filterContainer) {
                return;
              }
