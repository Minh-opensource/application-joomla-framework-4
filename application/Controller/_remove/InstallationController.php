...
class AlphaController extends JSONController
{
	...
	public function dbcheck()
	{
		...
		{
			$this->app->enqueueMessage(Text::_('TXT_DATABASE_VALIDATION_ERROR'), 'error');
			$r->validated = false;
	...
	public function languages()
	{
		...
		if (!$lids)
		{
			...
			$this->app->enqueueMessage(Text::_('TXT_LANGUAGES_NO_LANGUAGE_SELECTED'), 'warning');
		}
		else
		{
			...
			if (!empty($failedLanguages))
			{
				...
				{
					$this->app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_COULD_NOT_CREATE_CONTENT_LANGUAGE', $failedLanguage), 'warning');
				}
			}
		}
	...
	public function delete()
	{
		...
		if (!$success)
		{
			$this->app->enqueueMessage(Text::sprintf('TXT_COMPLETE_ERROR_FOLDER_DELETE', 'alpha'), 'warning');
		}
