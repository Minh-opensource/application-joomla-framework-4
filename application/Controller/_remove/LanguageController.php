...
class LanguageController extends JSONController
{
	...
	public function set()
	{
		...
		if ($session->isNew())
		{
			$this->sendJsonResponse(new \Exception(Text::_('TXT_COOKIES_NOT_ENABLED'), 500));
		}
	...
	public function setdefault()
	{
		...
		if (!$model->setDefault($admin_lang, 'administrator'))
		{
			...
			$this->app->enqueueMessage(Text::_('TXT_DEFAULTLANGUAGE_ADMIN_COULDNT_SET_DEFAULT'), 'error');
		}
		else
		{
			// Create a response body.
			$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_ADMIN_SET_DEFAULT', $admin_lang), 'message');
		}
		...
		{
			// Create an error response message.
			$app->enqueueMessage(Text::_('TXT_DEFAULTLANGUAGE_FRONTEND_COULDNT_SET_DEFAULT'), 'error');
		}
		else
		{
			// Create a response body.
			$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_FRONTEND_SET_DEFAULT', $frontend_lang), 'message');
		}
		...
		if ((int) $data['activateMultilanguage'])
		{
			...
			{
				$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_COULD_NOT_ENABLE_PLG_LANGUAGEFILTER', $frontend_lang), 'warning');
			}
			...
			{
				$app->enqueueMessage(Text::_('TXT_DEFAULTLANGUAGE_COULD_NOT_ENABLE_PLG_LANGUAGECODE'), 'warning');
			}

			if (!$model->addModuleLanguageSwitcher())
			{
				$app->enqueueMessage(Text::_('TXT_DEFAULTLANGUAGE_COULD_NOT_ENABLE_MODULESWHITCHER_LANGUAGECODE'), 'warning');
			}
			...
			foreach ($siteLanguages as $siteLang)
			{
				...
				{
					$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_COULD_NOT_CREATE_MENU', $siteLang->name), 'warning');
				...
				if (!$data['installLocalisedContent'])
				{
					...
					{
						$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_COULD_NOT_CREATE_MENU_ITEM', $siteLang->name), 'warning');
				...
				{
					$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_COULD_NOT_CREATE_MENU_MODULE', $frontend_lang), 'warning');
				...
				if ((int) $data['installLocalisedContent'])
				{
					...
					{
						$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_COULD_NOT_CREATE_CATEGORY', $frontend_lang), 'warning');
					...
					{
						$app->enqueueMessage(Text::sprintf('TXT_DEFAULTLANGUAGE_COULD_NOT_CREATE_ARTICLE', $frontend_lang), 'warning');
			...
			{
				$app->enqueueMessage(Text::_('TXT_DEFAULTLANGUAGE_COULD_NOT_ADD_ASSOCIATIONS'), 'warning');
			}

			if (!$model->disableModuleMainMenu())
			{
				$app->enqueueMessage(Text::_('TXT_DEFAULTLANGUAGE_COULD_NOT_UNPUBLISH_MOD_DEFAULTMENU'), 'warning');
			}

			if (!$model->enableModule('mod_multilangstatus'))
			{
				$app->enqueueMessage(Text::_('TXT_DEFAULTLANGUAGE_COULD_NOT_PUBLISH_MOD_MULTILANGSTATUS'), 'warning');
			}
		}
